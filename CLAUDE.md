...
- **VIX** — CBOE index options (OPT, not FOP): `live: 'ib'` but no `main.py` row, so the proxy answers 404 / `connected:false` and the deployed site reads it from `ib-eod.js` only. Each expiry prices off the VX future that settles with it (Black-76 on that future); the headline spot is the VIX index level. Strong call skew; IV lives at 40–200%.
