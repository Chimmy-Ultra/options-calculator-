...
- **K-line zoom / pan** (`PriceChart`): a `view` window into `bars`; the wheel zooms about the cursor, dragging pans, the legend shows `N/M 根 · 重設`, and the view resets when the series changes. MA / RSI keep running over the whole series and are clipped to the plot, so they stay correct at the window edges; the probability cone only draws at the newest bar. Level lines stay at their true price while their labels and right-axis tags are pushed apart by `spreadTags` (leader lines mark any that moved) — never let tags stack, that was the bug.
