...
const HELP_COPY = {
  ...
  theo: { term: '理論價 (theoretical price)', short: 'The product\'s own model — Black-Scholes on spot for TXO, Black-76 on the future for the IB products — priced at one reference volatility for the whole chain, next to the market (bid/ask mid, else last). 價平 IV: the reference is this expiry\'s ATM implied volatility, so the difference at each strike is its skew premium. 20日 HV: the reference is the underlying\'s 20-day realized volatility, so the difference is what the market charges over what the underlying has actually moved. + (gold) = market above the model, − (blue) = below.', example: 'ES 7,650 call · 價平 IV 15.2% → 理論價 71.5 · 市價 74.0 → +2.5' },
  kzoom: { term: 'K 線縮放 (zoom and pan)', short: 'The wheel zooms the K-line about the cursor and dragging pans; the legend shows how many of the loaded bars are on screen and 重設 goes back to all of them. Moving averages and RSI are still computed over the whole series, so the lines stay correct at the edges of the window instead of starting late. The probability cone only draws when the view is at the newest bar. Switching product, period or session resets the view.' },
