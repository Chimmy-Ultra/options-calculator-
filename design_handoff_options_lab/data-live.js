...
(function () {
  ...
  // per-strike close / bid-ask / settlement / OI, real TX daily bars.
  // ib-eod.js (generated by `server/ibsnap.py`) defines window.IB_EOD: the same
  // shape per IB product (ES / NQ / VIX / grains / energy), keyed by product id.
  // When the proxy is absent (the deployed site, a fresh checkout) every call
  // below answers from them instead of null, so the app shows real
  // previous-session numbers rather than mock. Stale snapshot (every expiry
  // passed) → ignored.
  function dteOf(id) {
  ...
  function eod(pid) {
    const s = (window.TAIFEX_EOD && window.TAIFEX_EOD.product === pid) ? window.TAIFEX_EOD
  ...
  const live = {
    ...
    // A proxy that is up but not serving this product's source counts as no
    // proxy, so the snapshot (not mock) answers.
    probe: async (pid) => {
      const r = await get('/api/health' + (pid ? '?pid=' + encodeURIComponent(pid) : ''), 2000);
  ...
  fallback.probe = async (pid) => {
    const s = eod(pid);
    return s ? { connected: true, source: 'eod', label: s.label || ('TAIFEX ' + s.date.slice(5) + ' EOD'), asOf: s.date } : null;
  };
  fallback.quote = async (pid) => {
    const s = eod(pid);
    if (!s) return null;
    const last = s.spot.price, ref = s.spot.ref;
    return { symbol: s.symbol || 'TXO', month: (s.front && s.front.month) || s.asOf.slice(0, 6), last, bid: null, ask: null, close: ref,
  ...
  fallback.chain = async (pid, expiry) => {
    ...
    if (!c) return null;
    // IB chains carry their own underlying future per expiry (ES Sep options on ESU6, Dec on ESZ6).
    return { expiry, dte: dteOf(expiry), underlying: c.underlying || { month: s.asOf.slice(0, 6), price: s.spot.price }, rows: c.rows,
      oi: s.oi ? { source: 'taifex', date: s.date, prevDate: s.prevDate } : undefined };
  };
  fallback.bars = async (pid, opts) => {
    ...
    return { symbol: s.symbol || 'TX', month: (s.front && s.front.month) || s.asOf.slice(0, 6), bar: '1 day', session: full ? 'full' : 'day', bars: full ? s.barsFull : s.bars };
  };
  fallback.oi = async (pid, expiry) => { const s = eod(pid); return (s && s.oi) ? eodOi(s, expiry) : null; };
